...
static void gl_item_updated(recdata_s *rd) {
   ...
   //snprintf(buf, 256,"%d %d",hour, min);

   //sprintf(buf, "%d/%d", hour, min);

   //snprintf(buf, "INSERT INTO feeding VALUES(%d,%d);", hour, min);
   //buf += sprintf(buf, " %d", min);
   int j = 0;
   j = sprintf(buf, "%d", hour);
   j += sprintf(buf + j, ":%d", min);
   //dlog_print(DLOG_INFO,"clock","%I %d",saved_time.tm_hour, saved_time.tm_min);
   dlog_print(DLOG_INFO, "tag", buf);

   saved_time.tm_min = min;
   dlog_print(DLOG_INFO, "saved time", "%d %d", saved_time.tm_hour,
         saved_time.tm_min);
...
static int db_read_cb(void *counter, int argc, char **argv, char **azColName) {
   ...
   dlog_print(DLOG_INFO, "tag", "%d %d", rd->tm_hour, rd->tm_min);

   gl_item_updated(rd);

   //elm_list_item_append(list2, buff, NULL, NULL, NULL, (void*) rd);
   //elm_list_go(list2);
...
static void popup_set_btn_clicked_cb(void *data, Evas_Object *obj,
   ...
   //dlog_print(DLOG_INFO,"save_time",saved_time.tm_hour);A
